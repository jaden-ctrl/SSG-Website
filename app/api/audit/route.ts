...
const failureCode=(cause:unknown)=>{const message=cause instanceof Error?cause.message.toLowerCase():"";if(message.includes("model"))return"MODEL_UNAVAILABLE";if(message.includes("api key")||message.includes("401")||message.includes("authentication"))return"MODEL_AUTH_FAILED";if(message.includes("timeout")||message.includes("timed out"))return"MODEL_TIMEOUT";return"ANALYSIS_FAILED"};
...
export async function POST(request:Request){
  ...
  }catch(cause){if(cause instanceof ZodError)return NextResponse.json({error:"Please check the form and try again.",fields:cause.flatten().fieldErrors,caseId},{status:400});const code=failureCode(cause);console.error(JSON.stringify({event:"audit_case_failed",caseId,durationMs:Date.now()-started,code,message:cause instanceof Error?cause.message:"unknown"}));return NextResponse.json({error:"SSGAI could not complete the analysis. The case is held for safe follow-up.",code,caseId},{status:500})}
}
