...
import { runAtlasFreeAudit } from "../atlas/free-audit";
import { captureLead, type LeadRecord } from "../actions/leads";
import { cases, transition, type AuditCase } from "../cases/repository";
...
const failureCode = (cause: unknown) => {
  ...
  if (name.includes("MaxTurns")) return "MODEL_MAX_TURNS";
  if (name.includes("ModelBehavior")) return "MODEL_OUTPUT_INVALID";
  if (name.includes("ModelRefusal")) return "MODEL_REFUSED";
  if (name.includes("Timeout") || message.includes("timeout") || message.includes("timed out")) return "MODEL_TIMEOUT";
  if (message.includes("schema") || message.includes("response_format") || message.includes("structured output")) return "MODEL_SCHEMA_INVALID";
  if (message.includes("api key") || message.includes("401") || message.includes("authentication")) return "MODEL_AUTH_FAILED";
  if (message.includes("insufficient_quota") || message.includes("quota") || message.includes("billing")) return "MODEL_QUOTA_EXHAUSTED";
  if (message.includes("429") || message.includes("rate limit")) return "MODEL_RATE_LIMITED";
  if (message.includes("model")) return "MODEL_UNAVAILABLE";
  if (message.includes("blob") || message.includes("store")) return "STORAGE_FAILED";
...
export async function handleAuditSubmit(request: Request) {
  ...
  try {
    if (!process.env.OPENAI_API_KEY) return json({ error: "The Free Audit service is not configured for this deploy preview. No submission was saved.", caseId }, 503);
    if (Number(request.headers.get("content-length") || 0) > 30_000) return json({ error: "Request is too large.", caseId }, 413);
    ...
    const lead: LeadRecord = { ...intake, id: caseId, createdAt: now, status: "received" };
