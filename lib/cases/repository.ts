...
class NetlifyCases implements CaseRepository{private get store(){return getStore("ssgai-cases")}async save(record:AuditCase){await this.store.setJSON(record.caseId,record)}async get(caseId:string){return await this.store.get(caseId,{type:"json"}) as AuditCase|null}}

function repository():CaseRepository{return process.env.NETLIFY?new NetlifyCases():new LocalCases()}
