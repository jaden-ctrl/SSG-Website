...
class NetlifyCases implements CaseRepository{private get store(){return openNetlifyStore("ssgai-cases")}async save(record:AuditCase){await this.store.setJSON(record.caseId,record)}async get(caseId:string){return await this.store.get(caseId,{type:"json",consistency:"strong"}) as AuditCase|null}}

function repository():CaseRepository{return storageBackend()==="netlify"?new NetlifyCases():new LocalCases()}
export const cases:CaseRepository={
  save(record){return repository().save(record)},
  get(caseId){return repository().get(caseId)}
};
